  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      if (!isDeleting) {
        ...
        if (charIndex + 1 === currentWord.length) setTimeout(() => setIsDeleting(true), pause);
      } else {
        setDisplay(currentWord.slice(0, charIndex - 1));
        setCharIndex(prev => prev - 1);
        if (charIndex === 0) { setIsDeleting(false); setWordIndex(prev => (prev + 1) % words.length); }
      }
...
const AnimatedCounter = ({ target, suffix = "" }: { target: number; suffix?: string }) => {
  ...
  useEffect(() => {
    if (!inView) return;
    let start = 0;
    const step = () => { start += target / 40; if (start >= target) { setCount(target); return; } setCount(Math.floor(start)); requestAnimationFrame(step); };
    step();
  }, [inView, target]);
    ...
    <section ref={sectionRef} className="relative min-h-screen flex items-center overflow-hidden">
      ...
      <motion.div className="relative z-10 max-w-7xl mx-auto w-full section-padding">
          ...
          <div className="lg:col-span-7 space-y-7">
            ...
            <div className="space-y-1" style={{ perspective: "1000px" }}>
              <div className="overflow-hidden pb-2">
                <motion.h1 variants={letterReveal} className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold leading-[1] tracking-[-0.04em] text-foreground/60"
                  style={{ fontFamily: "var(--font-display)" }}>Hi, I'm</motion.h1>
              </div>
              <div className="overflow-hidden pb-2">
                  ...
                  {nameLetters.map((letter, i) => (
                    <motion.span key={i}
                      ...
                      style={{
                        ...
                        background: "linear-gradient(135deg, hsl(265, 90%, 65%), hsl(185, 100%, 55%), hsl(35, 95%, 60%))",
                        backgroundSize: "200% auto", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent",
                        backgroundClip: "text", animation: "text-shimmer 4s linear infinite",
                      }}
              ...
              <div className="overflow-hidden pb-2">
                <motion.h1 variants={letterReveal}
                  ...
                  style={{ fontFamily: "var(--font-display)" }}>Singh.</motion.h1>
              </div>
            </div>

            <motion.div variants={fadeUp} className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-violet-400 animate-pulse-glow" />
              <p className="text-lg md:text-xl text-muted-foreground" style={{ fontFamily: "var(--font-mono)" }}>
                {roleText}
                <motion.span className="inline-block w-[2px] h-5 bg-violet-400 ml-1 align-middle"
                  animate={{ opacity: [1, 0] }} transition={{ duration: 0.6, repeat: Infinity, repeatType: "reverse" }} />
              </p>
            </motion.div>

            <motion.p variants={fadeUp} className="text-muted-foreground max-w-lg leading-relaxed text-base">
              Crafting digital experiences with modern technologies. Passionate about creating beautiful, functional, and user-centered applications that make a difference.
            </motion.p>
            ...
            <motion.div variants={fadeUp} className="flex flex-wrap gap-4 pt-2">
              ...
              <MagneticButton onClick={downloadResume}
            ...
            <motion.div variants={fadeUp} className="flex gap-3 pt-2">
              {[
                { icon: Github, href: "https://github.com/Aditya-singh-AI", label: "GitHub" },
                { icon: Linkedin, href: "#", label: "LinkedIn" },
                { icon: Twitter, href: "#", label: "Twitter" },
                { icon: Mail, href: "mailto:aditya.asb24@gmail.com", label: "Email" },
              ].map((s, i) => (
            ...
            <motion.div className="relative">
              ...
              {[
                ...
                <motion.div key={i} className="absolute w-10 h-10 rounded-xl glass-effect flex items-center justify-center z-20"
                  ...
                  transition={{ opacity: { delay: b.delay }, scale: { delay: b.delay, type: "spring" }, y: { delay: b.delay + 0.5, duration: 3, repeat: Infinity } }}>
        ...
        <motion.div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-3"
          ...
          <span className="text-[10px] tracking-[0.25em] uppercase text-muted-foreground" style={{ fontFamily: "var(--font-mono)" }}>Explore</span>
        </motion.div>
      </motion.div>
    </section>
  );
